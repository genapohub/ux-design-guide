...
- **执行后（硬性要求，不可跳过）**：追加 `[ux-design-guide] 场景描述 → 关键决策` 到 `.skills-memory/YYYY-MM-DD.md`；如有可复用决策，去重后追加到 MEMORY.md 对应分段。**记忆写入是交付物的一部分**——如果因环境限制无法写入，必须在最终回复中明确告知用户「记忆未写入」及原因，不得静默跳过
- **轮转检查**：
...
- 间距遵循 4pt/8pt 栅格体系
- **必须读取并应用"十、超越AI味：设计灵魂注入指南"**：在产出设计方案时，逐条对照 10.1 反模板化十条，注入品牌气质矩阵（10.2），应用视觉张力语言（10.3），映射情感节奏（10.4），使用真实内容思维做设计（10.5），并参考行业精度（10.6）确保方案符合所在行业的"设计母题"
- 产出后保存为 Markdown 文件
...
- AI 工具使用建议与提示词模板
- 产出质量检查清单（含去AI味检查）
- **超越AI味：设计灵魂注入指南**（反模板化十条/品牌气质矩阵/视觉张力/情感节奏/真实内容思维/行业精度）

在 Step 2（场景识别）、Step 4（产出方案）、Step 5（质量检查）时必须读取对应章节。文档按需读取：
- 场景识别：读取"一、场景识别"
- 产出方案：读取对应场景章节（二至六）+ "七、通用设计交付标准" + "十、超越AI味：设计灵魂注入指南"
- 质量检查：读取"九、产出质量检查清单"
